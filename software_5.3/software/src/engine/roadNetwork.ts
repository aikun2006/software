...
/**
 * 英文名 → 中文名映射表：英文模式下用户看到的景点名 → 路网中存储的中文名。
 * 用于 findNodeByName() 在英文模式下也能正确定位路网节点。
 */
const EN_TO_ZH_NAME_MAP: Record<string, string> = {
  'Ticket Gate': '检票口',
  'Tourist Center': '游客中心',
  'Holiday Plaza': '假日广场',
  'Guide Service': '导游服务',
  'Bodhi Avenue': '菩提大道',
  'Buddha Footprints Plaza': '梵足坛',
  'Nine Dragons Bathing': '九龙灌浴',
  'Five Seal Mandala': '五印坛城',
  'Scenic Exit': '景区出口',
  'Lingshan Buddhist Palace': '灵山梵宫',
  'Buddha Palm': '灵山佛手',
  'Wish-Fulfilling Garden': '祈福普缘',
  'Manfeilong Pagoda': '曼飞龙塔',
  'Multi-Treasure Pagoda': '灵山多宝塔',
  'Lingshan Jingshe': '灵山精舍',
  'Vegetarian Restaurant': '蔬食馆',
  'Hundred Children Playing with Maitreya': '百子戏弥勒',
  'Xiangfu Temple': '祥符禅寺',
  'Shengfan Zhai': '盛凡斋',
  'Xingtan Plaza': '杏坛广场',
  'Grand Buddha': '灵山大佛',
  'Buddha Base': '佛座',
  'Endless Compassion Hall': '无尽意斋',
  'Buddhist Culture': '佛教文化',
  'Life Release Pond': '放生礼佛',
  'Ashoka Pillar': '阿育王柱',
}

/**
 * 中文名 → 英文名映射表（路网节点）：英文模式下将路网中存储的中文名翻译为英文。
 * 与 EN_TO_ZH_NAME_MAP 互逆，供导航指令、地标列表等场景使用。
 */
const ZH_TO_EN_NODE_NAME_MAP: Record<string, string> = {
  '检票口': 'Ticket Gate',
  '游客中心': 'Tourist Center',
  '假日广场': 'Holiday Plaza',
  '导游服务': 'Guide Service',
  '菩提大道': 'Bodhi Avenue',
  '梵足坛': 'Buddha Footprints Plaza',
  '佛足坛': 'Buddha Footprints Plaza',
  '九龙灌浴': 'Nine Dragons Bathing',
  '五印坛城': 'Five Seal Mandala',
  '景区出口': 'Scenic Exit',
  '灵山梵宫': 'Lingshan Buddhist Palace',
  '灵山佛手': 'Buddha Palm',
  '祈福普缘': 'Wish-Fulfilling Garden',
  '曼飞龙塔': 'Manfeilong Pagoda',
  '飞龙塔': 'Manfeilong Pagoda',
  '曼龙飞塔': 'Manfeilong Pagoda',
  '灵山多宝塔': 'Multi-Treasure Pagoda',
  '多宝塔': 'Multi-Treasure Pagoda',
  '灵山精舍': 'Lingshan Jingshe',
  '蔬食馆': 'Vegetarian Restaurant',
  '百子戏弥勒': 'Hundred Children Playing with Maitreya',
  '祥符禅寺': 'Xiangfu Temple',
  '盛凡斋': 'Shengfan Zhai',
  '杏坛广场': 'Xingtan Plaza',
  '灵山大佛': 'Grand Buddha',
  '佛座': 'Buddha Base',
  '无尽意斋': 'Endless Compassion Hall',
  '佛教文化': 'Buddhist Culture',
  '放生礼佛': 'Life Release Pond',
  '阿育王柱': 'Ashoka Pillar',
  '景区出口假日广场': 'Holiday Plaza',
  '桥': 'Bridge',
}

/**
 * 中文名 → 英文名映射表（道路名）：英文模式下将"景区N路"等路名翻译为英文。
 */
const ZH_TO_EN_ROAD_NAME_MAP: Record<string, string> = {
  '景区一路': 'Scenic Road 1',
  '景区二路': 'Scenic Road 2',
  '景区三路': 'Scenic Road 3',
  '新路段': 'Walkway',
...
/**
 * 多语言节点名翻译：将路网中存储的中文名按界面语言翻译。
 * 中文模式原样返回，英文模式查表翻译，未命中时返回原名。
 */
export function translateNodeName(name: string, lang: 'zh' | 'en' = 'zh'): string {
  if (lang === 'zh' || !name) return name
  return ZH_TO_EN_NODE_NAME_MAP[name] || name
}

/**
 * 多语言道路名翻译：将路网中存储的中文路名按界面语言翻译。
 * 中文模式原样返回，英文模式查表翻译，未命中时返回原名。
 */
export function translateRoadName(name: string, lang: 'zh' | 'en' = 'zh'): string {
  if (lang === 'zh' || !name) return name
  return ZH_TO_EN_ROAD_NAME_MAP[name] || name
}

/** 根据名称查找路网节点（支持中英文） */
export function findNodeByName(name: string): RoadNode | undefined {
  // 先尝试英文名 → 中文名映射
  const zhName = EN_TO_ZH_NAME_MAP[name]
  const searchName = zhName || name
  return getAllNodes().find(n => n.name === searchName || n.name.includes(searchName))
}
