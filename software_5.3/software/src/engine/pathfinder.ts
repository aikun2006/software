...
/**
 * 多语言路段指令格式化：根据语言生成 "沿X路走Y米" / "Go along X for Ym" 等指令。
 * 供 navigation.vue 和 navigationEngine.ts 共用，确保指令语言与界面语言一致。
 * 英文模式下自动将中文路名/节点名翻译为英文，避免出现 "Follow 景区一路 for 100m" 的混合文本。
 */
export function formatSegmentInstruction(seg: RouteSegment, lang: 'zh' | 'en' = 'zh'): string {
  const distM = Math.max(1, Math.round(seg.distance))
  if (lang === 'en') {
    // 有路名 → "Follow <road> for Xm"（路名翻译为英文）
    if (seg.roadName) {
      const roadEn = translateRoadName(seg.roadName, 'en')
      return `Follow ${roadEn} for ${distM}m`
    }
    // 无路名 → 转弯指令（目标节点名翻译为英文）
    const target = translateNodeName(seg.toName || '', 'en')
    switch (seg.turnDirection) {
      case 'left': return `Turn left toward ${target}`
      case 'right': return `Turn right toward ${target}`
      case 'slight-left': return `Slight left toward ${target}`
      case 'slight-right': return `Slight right toward ${target}`
      case 'u-turn': return `U-turn toward ${target}`
      default: return target ? `Continue toward ${target}` : 'Continue straight'
    }
  }
  // 中文：复用原有逻辑
  if (seg.roadName) {
    return `沿${seg.roadName}走${distM}米`
  }
  return turnToText(seg.turnDirection || 'straight', seg.toName || '')
}
 ...
 * 用于路线模式在导航页展示"途经景区名"。
 * @param lang 界面语言，英文模式下将中文节点名翻译为英文
 */
export function getLandmarksOnPath(nodeIds: string[], lang: 'zh' | 'en' = 'zh'): { id: string; name: string }[] {
  const result: { id: string; name: string }[] = []
  let lastName = ''
  for (const id of nodeIds) {
    const n = getNodeById(id)
    if (!n || !n.name) continue
    const displayName = translateNodeName(n.name, lang)
    if (displayName === lastName) continue
    result.push({ id: n.id, name: displayName })
