<template>
  <view class="admin-container">
    ...
    <view class="menu-grid">
      ...
      <view class="menu-item" @click="goTo('/pages/admin/ai-models')">
        <view class="menu-icon ai-models">
          <text>🤖</text>
        </view>
        <text class="menu-title">AI模型配置</text>
        <text class="menu-desc">多模型统一调度管理</text>
      </view>
    </view>
...
const handleLogout = () => {
  uni.showModal({
    ...
    success: (res) => {
      if (res.confirm) {
        userStore.logout()
        // 直接跳转应用首页（tabBar 页），清空管理后台页面栈，不回退到 admin/login
        uni.switchTab({ url: '/pages/visitor/index' })
      }
    }
  })
}
