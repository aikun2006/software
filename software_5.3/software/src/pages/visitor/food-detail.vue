<template>
  <view class="detail-page">
    ...
    <view class="detail-header">
      ...
      <text class="header-title">{{ shop ? (currentLang === 'zh' ? shop.name : shop.nameEn || shop.name) : (currentLang === 'zh' ? '美食详情' : 'Food Details') }}</text>
    ...
    <scroll-view v-if="shop" class="detail-scroll" scroll-y>
      ...
      <view class="section-card" v-if="shop.fullDesc">
        <text class="section-title">{{ currentLang === 'zh' ? '店铺介绍' : 'About the Restaurant' }}</text>
      ...
      <view class="section-card tips-card" v-if="shop.tips">
        <text class="section-title">💡 {{ currentLang === 'zh' ? '就餐贴士' : 'Dining Tips' }}</text>
      ...
      <view class="post-card">
        <template v-if="userStore.isLoggedIn">
          ...
          <textarea
            ...
            v-model="myContent"
            :placeholder="currentLang === 'zh' ? '分享你的就餐感受（最多500字）' : 'Share your dining experience (max 500 chars)'"
        ...
        <template v-else>
          <text class="login-tip">{{ currentLang === 'zh' ? '登录后即可发表评价，分享你的就餐感受' : 'Log in to write a review and share your experience' }}</text>
...
onLoad((options: any) => {
  ...
  if (!shop.value) {
    uni.showToast({ title: currentLang.value === 'zh' ? '店铺不存在' : 'Restaurant not found', icon: 'none' })
