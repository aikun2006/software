  ...
  <view class="spots-page">
    ...
    <view class="spots-header">
      <view class="title-row">
        ...
        <text class="page-title">{{ currentLang === 'zh' ? '住宿推荐' : 'Accommodation' }}</text>
      </view>
    <view class="search-box">
      ...
      <input
        class="search-input"
        v-model="keyword"
        :placeholder="currentLang === 'zh' ? '搜索酒店名 / 简介' : 'Search hotel name / description'"
    ...
    <!-- 住宿卡片列表（一行一个） -->
    <scroll-view class="spots-list" scroll-y :scroll-top="scrollTop" :show-scrollbar="false">
      <view
        class="spot-card"
      ...
      <view class="empty-tip" v-if="filteredHotels.length === 0">
        <text>{{ currentLang === 'zh' ? '没有找到相关住宿' : 'No hotels found' }}</text>
      </view>
...
import { hotels, type HotelShop } from '@/data/hotels'
import { currentLang } from '@/i18n'

const keyword = ref('')
const scrollTop = ref(0)

const filteredHotels = computed<HotelShop[]>(() => {
  if (!keyword.value.trim()) return hotels
...
const openDetail = (id: string) => {
  uni.navigateTo({ url: '/pages/visitor/hotel-detail?id=' + id })
}
...
.spots-page {
  height: 100vh;
  overflow: hidden;          // 全屏无右侧滚动条，内部 scroll-view 负责滚动
  background: $bg-color;
  display: flex;
...
.spots-list {
  flex: 1;
  min-height: 0;             // 打破 flex 子项 min-height:auto 默认值，让 scroll-view 可被父容器约束高度从而内部滚动
