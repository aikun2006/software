  ...
  <view class="home-screen" @touchmove.stop.prevent>
    ...
    <view class="avatar-area" @click="onAvatarClick">
      <view class="avatar-stage">
        ...
        <Avatar3D ref="avatarRef" :show-name="false" :disable-zoom="true" :vertical-offset-ratio="1/4" :model-path="activeAvatarConfig.vrm_path" :model-scale="activeAvatarConfig.model_scale" :model-rotation-y="activeAvatarConfig.rotation_y" :name="activeAvatarConfig.name" class="enlarged-avatar" />
      </view>
    ...
    <view class="notice-overlay" v-if="showNotice">
      <view class="notice-popup" @click.stop>
        <!-- 右上角按钮组：English 切换 + 关闭 -->
        <view class="notice-top-actions">
          <view class="notice-lang-btn" @click="onToggleLang">
            <text class="notice-lang-text">{{ currentLang === 'zh' ? 'English' : '中文' }}</text>
          </view>
...
// D5：当前启用的数字人形象配置（从后端获取，控制 VRM 模型路径/缩放/旋转/名字）
const activeAvatarConfig = ref<{
  vrm_path: string
  model_scale: number
  rotation_y: number
  name: string
}>({ vrm_path: '', model_scale: 3.25, rotation_y: 0, name: '小乐' })

// D5：获取当前启用的数字人形象配置（每次 onShow 都请求后端，确保切换数字人后首页立即更新）
const AVATAR_CACHE_KEY = 'lingshan_avatar_cache'

// 应用配置到响应式状态（仅在变化时更新，避免不必要的模型重载）
// name 变化也需更新（切换数字人时名字跟随刷新；模型重载由 Avatar3D 内部 watch(modelPath) 控制）
function applyAvatarConfig(newConfig: { vrm_path: string; model_scale: number; rotation_y: number; name: string }) {
...
async function fetchActiveAvatar() {
  // 每次都请求后端获取最新配置，确保管理后台切换数字人后首页立即更新
  try {
...
// 首次播放与循环恢复分离：返回首页时不再重放首次 3 秒提示音
let audioFirstPlayed = false

// 首次启动：3 秒后播放第一次提示音 + 15 秒循环（仅 onMounted 触发一次）
const startAudioLoop = () => {
  if (audioStarted) return
  audioStarted = true
  firstPlayTimer = setTimeout(() => {
    playPromptAudio()
    audioFirstPlayed = true   // 标记首次已播放，后续 onShow 不再重放
  }, 3000)
...
// 恢复循环：仅重启 15 秒定时器，不重放首次 3 秒提示音（onShow 调用）
const resumeAudioLoop = () => {
  if (audioStarted) return
  audioStarted = true
  promptTimer = setInterval(() => {
    playPromptAudio()
...
const onCardClick = (idx: number) => {
  ...
  if (card.type === 'vue') {
    ...
    // 静态 HTML（购票/天气）跳转到全屏 webview 页面，不离开 SPA
    uni.navigateTo({ url: '/pages/visitor/webview?url=' + encodeURIComponent(card.link) })
  }
}
...
onMounted(() => {
  ...
  if (!sessionStorage.getItem('lingshan_notice_shown')) {
...
onShow(() => {
  ...
  // D5：页面重新显示时刷新数字人配置（带缓存，命中则零网络请求）
  fetchActiveAvatar()
})
...
// ============ 新增：用户须知弹窗 ============
// 用 sessionStorage 持久化：SPA会话内只弹一次，刷新后重新弹出
// （组件被 KeepAlive 重建时模块级变量会重置，sessionStorage 能跨重建保持）
...
const closeNotice = () => {
  ...
  sessionStorage.setItem('lingshan_notice_shown', '1')
}
...
.avatar-stage {
  ...
  aspect-ratio: 3 / 4;
  /* 视觉融合：圆角 + 渐变背景 + 阴影，使光晕与数字人形成统一整体（边框已移除） */
  border-radius: 24rpx;
...
/* 公共样式：圆角浅底色，放大字体方便视力不佳用户识别大字版入口 */
.lang-toggle-btn,
.font-toggle-btn {
  height: 76rpx;
  padding: 0 32rpx;
...
@media screen and (max-width: 768px) {
  ...
  /* 弹窗内 English 按钮：复用首页 lang-toggle-btn 视觉风格 */
  .notice-lang-btn {
    height: 48rpx;
    padding: 0 20rpx;
    display: flex;
