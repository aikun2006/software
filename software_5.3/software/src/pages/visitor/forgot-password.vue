    ...
    <view class="auth-card">
      ...
      <view v-if="step === 1">
        <view class="form-item">
          ...
          <input class="input fp-phone-input" v-model="form.phone" type="number" :placeholder="isZh ? '请输入注册手机号' : 'Enter registered mobile number'" maxlength="11" @confirm="onPhoneConfirm" />
        </view>

        <view class="form-item">
          <text class="label">{{ isZh ? '验证码' : 'Verification Code' }}</text>
          <view class="code-row">
            <input class="input code-input fp-code-input" v-model="form.code" type="number" :placeholder="isZh ? '6 位验证码' : '6-digit code'" maxlength="6" @confirm="onCodeConfirm" />
      ...
      <view v-else-if="step === 2">
        <view class="form-item">
          ...
          <input class="input fp-newpwd-input" v-model="form.newPassword" type="password" :placeholder="isZh ? '至少 6 位' : 'At least 6 characters'" @confirm="onNewPwdConfirm" />
        </view>

        <view class="form-item">
          <text class="label">{{ isZh ? '确认新密码' : 'Confirm Password' }}</text>
          <input class="input fp-confirmpwd-input" v-model="form.confirmPassword" type="password" :placeholder="isZh ? '再次输入新密码' : 'Re-enter new password'" @confirm="onConfirmPwdConfirm" />
        </view>
...
// 步骤1：手机号回车 → 聚焦验证码
const onPhoneConfirm = () => focusField('fp-code-input')
// 步骤1：验证码回车 → 触发验证
const onCodeConfirm = () => handleVerifyCode()
// 步骤2：新密码回车 → 聚焦确认密码
const onNewPwdConfirm = () => focusField('fp-confirmpwd-input')
// 步骤2：确认密码回车 → 触发重置
const onConfirmPwdConfirm = () => handleResetPassword()
