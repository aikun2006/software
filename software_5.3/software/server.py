...
def init_db():
    ...
    with DB_LOCK:
        conn = _db()
        # ===== 数据加密迁移：回填历史明文数据为加密数据 =====
        try:
            # 检查是否需要给 users 表加 phone_hmac 列（兼容旧库）
            cols = [r[1] for r in conn.execute('PRAGMA table_info(users)').fetchall()]
            if 'phone_hmac' not in cols:
                conn.execute('ALTER TABLE users ADD COLUMN phone_hmac TEXT')
                print('[迁移] users 表新增 phone_hmac 列')

            # 检查是否需要给 chat_logs 表加 user_id 列（兼容旧库）
            cols2 = [r[1] for r in conn.execute('PRAGMA table_info(chat_logs)').fetchall()]
            if 'user_id' not in cols2:
                conn.execute('ALTER TABLE chat_logs ADD COLUMN user_id INTEGER')
                print('[迁移] chat_logs 表新增 user_id 列')

            # 检查 admin_users 表是否存在
            tbls = [r[0] for r in conn.execute("SELECT name FROM sqlite_master WHERE type='table'").fetchall()]
            if 'admin_users' not in tbls:
                ...
                print('[迁移] 新增 admin_users 表')

            # 回填 users.phone_hmac（对未加密的旧 phone 生成盲索引）
            rows_to_migrate = conn.execute(
                "SELECT id, phone FROM users WHERE phone_hmac IS NULL OR phone_hmac=''"
            ).fetchall()
            for r in rows_to_migrate:
                ph = r['phone']
                # 判断 phone 是否已加密（Fernet 密文以 gAAAA 开头）
                if ph and not ph.startswith('gAAAA'):
                    # 旧明文 phone：生成 HMAC + 加密 phone
                    conn.execute(
                        'UPDATE users SET phone=?, phone_hmac=? WHERE id=?',
                        (encrypt_field(ph), phone_hmac(ph), r['id'])
                    )
                else:
                    # phone 已加密但 phone_hmac 为空（无法从密文还原 HMAC，跳过）
                    pass
            if rows_to_migrate:
                print(f'[迁移] users.phone_hmac 回填 {len(rows_to_migrate)} 条')

            # 回填 users.age（旧 INTEGER 转为加密 TEXT）
            age_rows = conn.execute(
                "SELECT id, age FROM users WHERE age IS NOT NULL AND age != '' AND age NOT LIKE 'gAAAA%'"
            ).fetchall()
            for r in age_rows:
                conn.execute('UPDATE users SET age=? WHERE id=?', (encrypt_field(str(r['age'])), r['id']))
            if age_rows:
                print(f'[迁移] users.age 加密回填 {len(age_rows)} 条')

            # 回填 users.nickname
            nick_rows = conn.execute(
                "SELECT id, nickname FROM users WHERE nickname IS NOT NULL AND nickname != '' AND nickname NOT LIKE 'gAAAA%'"
            ).fetchall()
            for r in nick_rows:
                conn.execute('UPDATE users SET nickname=? WHERE id=?', (encrypt_field(r['nickname']), r['id']))
            if nick_rows:
                print(f'[迁移] users.nickname 加密回填 {len(nick_rows)} 条')

            # 回填 users.gender
            gender_rows = conn.execute(
                "SELECT id, gender FROM users WHERE gender IS NOT NULL AND gender != '' AND gender NOT LIKE 'gAAAA%'"
            ).fetchall()
            for r in gender_rows:
                conn.execute('UPDATE users SET gender=? WHERE id=?', (encrypt_field(r['gender']), r['id']))
            if gender_rows:
                print(f'[迁移] users.gender 加密回填 {len(gender_rows)} 条')

            # 回填 chat_logs（旧明文对话加密）
            chat_rows = conn.execute(
                "SELECT id, user_question, ai_answer FROM chat_logs WHERE user_question NOT LIKE 'gAAAA%'"
            ).fetchall()
            for r in chat_rows:
                conn.execute(
                    'UPDATE chat_logs SET user_question=?, ai_answer=? WHERE id=?',
                    (encrypt_field(r['user_question']), encrypt_field(r['ai_answer']), r['id'])
                )
            if chat_rows:
                print(f'[迁移] chat_logs 加密回填 {len(chat_rows)} 条')

            # 回填 spot_reviews.nickname 和 content
            rev_nick = conn.execute(
                "SELECT id, nickname FROM spot_reviews WHERE nickname NOT LIKE 'gAAAA%'"
            ).fetchall()
            for r in rev_nick:
                conn.execute('UPDATE spot_reviews SET nickname=? WHERE id=?', (encrypt_field(r['nickname']), r['id']))

            rev_content = conn.execute(
                "SELECT id, content FROM spot_reviews WHERE content NOT LIKE 'gAAAA%'"
            ).fetchall()
            for r in rev_content:
                conn.execute('UPDATE spot_reviews SET content=? WHERE id=?', (encrypt_field(r['content']), r['id']))
            if rev_nick or rev_content:
                print(f'[迁移] spot_reviews 加密回填 {len(rev_nick) + len(rev_content)} 条')

            # 回填 reset_codes.phone 和 code
            rc_rows = conn.execute(
                "SELECT phone, code FROM reset_codes WHERE phone NOT LIKE 'gAAAA%'"
            ).fetchall()
            for r in rc_rows:
                conn.execute(
                    'UPDATE reset_codes SET phone=?, code=? WHERE phone=?',
                    (encrypt_field(r['phone']), encrypt_field(r['code']), r['phone'])
                )

            # 回填 ai_models.api_key
            ak_rows = conn.execute(
                "SELECT id, api_key FROM ai_models WHERE api_key IS NOT NULL AND api_key != '' AND api_key NOT LIKE 'gAAAA%'"
            ).fetchall()
            for r in ak_rows:
                conn.execute('UPDATE ai_models SET api_key=? WHERE id=?', (encrypt_field(r['api_key']), r['id']))
            if ak_rows:
                print(f'[迁移] ai_models.api_key 加密回填 {len(ak_rows)} 条')

            # 初始化默认管理员（admin/admin123），使用 PBKDF2 哈希
            admin_exists = conn.execute('SELECT 1 FROM admin_users WHERE username=?', ('admin',)).fetchone()
            if not admin_exists:
                conn.execute(
                    'INSERT INTO admin_users (username, password_hash, created_at) VALUES (?,?,?)',
                    ('admin', hash_password('admin123'), time.strftime('%Y-%m-%d %H:%M:%S'))
                )
                print('[迁移] 已创建默认管理员 admin/admin123（PBKDF2 哈希入库）')

            conn.commit()
        except Exception as _migrate_err:
            print(f'[迁移警告] 数据迁移出错：{_migrate_err}')
            conn.commit()  # 确保已执行的迁移不回滚
        finally:
            conn.close()


# ===== 字段级加密（Fernet 对称加密 + HMAC 盲索引） =====
# 主密钥优先从环境变量 LINGSHAN_DB_KEY 加载；开发环境若未设置，自动生成并写入 .env 文件
import hmac as _hmac_mod
_DB_KEY = os.environ.get('LINGSHAN_DB_KEY')
if not _DB_KEY:
    ...
    if os.path.exists(_env_path):
        with open(_env_path, 'r', encoding='utf-8') as _f:
            for _line in _f:
                _line = _line.strip()
                if _line.startswith('LINGSHAN_DB_KEY='):
                    _DB_KEY = _line.split('=', 1)[1].strip()
                    break
    if not _DB_KEY:
        from cryptography.fernet import Fernet as _FGen
        _DB_KEY = _FGen.generate_key().decode()
        try:
            with open(_env_path, 'w', encoding='utf-8') as _f:
                _f.write(f'LINGSHAN_DB_KEY={_DB_KEY}\n')
            print(f'[加密] 已自动生成主密钥并写入 .env 文件：{_env_path}')
            print(f'[加密] 生产环境请通过环境变量 LINGSHAN_DB_KEY 设置固定密钥！')
        except Exception as _e:
            print(f'[加密警告] 无法写入 .env：{_e}，使用临时密钥（重启后无法解密）')

try:
    from cryptography.fernet import Fernet as _Fernet, InvalidToken as _InvalidToken
    _FERNET = _Fernet(_DB_KEY.encode())
except ImportError:
    print('[加密警告] cryptography 库未安装，字段加密降级为明文！请运行 pip install cryptography')
    _FERNET = None
    _InvalidToken = Exception

_HMAC_KEY = os.environ.get('LINGSHAN_HMAC_KEY', _DB_KEY).encode()

def encrypt_field(plaintext):
    """加密字段，返回 base64 字符串。None/空值原样返回。"""
...
def decrypt_field(ciphertext):
    """解密字段，返回明文。None/空值原样返回。解密失败时兼容旧明文数据。"""
...
def phone_hmac(phone):
    """生成手机号 HMAC 盲索引（确定性、可索引、不可逆）"""
    return _hmac_mod.new(_HMAC_KEY, phone.encode('utf-8'), hashlib.sha256).hexdigest()
...
def _user_public(row) -> dict:
    _phone = decrypt_field(row['phone'])
    return {
        'id': row['id'],
        'phone': _phone,
        'age': decrypt_field(row['age']),
        'nickname': decrypt_field(row['nickname']) or ('游客' + _phone[-4:]),
...
def _verify_admin_login(username: str, password: str) -> bool:
    """验证管理员账号密码（从 admin_users 表读取，PBKDF2 校验）"""
    conn = _db()
    try:
...
class ProxyHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def handle_user_register(self):
        ...
        _ph = phone_hmac(phone)
        with DB_LOCK:
            conn = _db()
            try:
                if conn.execute('SELECT 1 FROM users WHERE phone_hmac=?', (_ph,)).fetchone():
                    self.send_json(409, {'error': '该手机号已注册'}); return
                cur = conn.execute(
                    'INSERT INTO users (phone, phone_hmac, password_hash, age, nickname, gender, created_at) VALUES (?,?,?,?,?,?,?)',
                    (encrypt_field(phone), _ph, hash_password(password),
                     encrypt_field(str(age)) if age is not None else None,
                     encrypt_field(nickname) if nickname else None,
                     encrypt_field(gender) if gender else None,
                ...
                conn.close()
        print(f'游客注册: {phone_hmac(phone)[:8]}...')
    ...
    def handle_user_login(self):
        ...
        with DB_LOCK:
            conn = _db()
            try:
                row = conn.execute('SELECT * FROM users WHERE phone_hmac=?', (_ph,)).fetchone()
            finally:
                conn.close()
        if not row or not verify_password(password, row['password_hash']):
            self.send_json(401, {'error': '手机号或密码不正确'}); return
        token = _issue_token(row['id'])
        print(f'游客登录: {phone_hmac(phone)[:8]}...')
    ...
    def handle_user_send_code(self):
        ...
        with DB_LOCK:
            ...
            try:
                # 校验用户是否存在
                row = conn.execute('SELECT 1 FROM users WHERE phone_hmac=?', (phone_hmac(phone),)).fetchone()
                if not row:
                    self.send_json(404, {'error': '该手机号未注册'})
                    return
                # 校验重发间隔
                existing = conn.execute(
                    'SELECT created_at FROM reset_codes WHERE phone=?', (encrypt_field(phone),)
                ).fetchone()
                ...
                conn.execute(
                    ...
                    'VALUES (?,?,?,?,0)',
                    (encrypt_field(phone), encrypt_field(code), now, now + RESET_CODE_TTL)
                )
                conn.commit()
            finally:
                conn.close()
        # 生产环境此处应调用短信网关发送 code；当前为本地部署，打印日志即可
        print(f'[密码重置] 验证码已生成：{phone_hmac(phone)[:8]}...')
    ...
    def handle_user_reset_password(self):
        ...
        with DB_LOCK:
            ...
            try:
                rec = conn.execute(
                    'SELECT * FROM reset_codes WHERE phone=?', (encrypt_field(phone),)
                ).fetchone()
                # 验证码不存在或已过期
                if not rec or rec['expires_at'] < now:
                    if rec:
                        conn.execute('DELETE FROM reset_codes WHERE phone=?', (encrypt_field(phone),))
                        conn.commit()
                ...
                if rec['attempts'] >= RESET_CODE_MAX_ATTEMPTS:
                    ...
                    return
                # 验证码不匹配
                if not secrets.compare_digest(decrypt_field(rec['code']), code):
                    conn.execute(
                ...
                # 验证通过，更新密码
                conn.execute(
                    'UPDATE users SET password_hash=? WHERE phone_hmac=?',
                    (hash_password(new_password), phone_hmac(phone))
                )
                # 删除已使用的验证码
                conn.execute('DELETE FROM reset_codes WHERE phone=?', (encrypt_field(phone),))
                conn.commit()
                ...
                conn.close()
        print(f'[密码重置] 密码已重置：{phone_hmac(phone)[:8]}...')
        self.send_json(200, {'ok': True})
    ...
    def handle_reviews_list(self):
        ...
        reviews = [{
            ...
            'nickname': decrypt_field(r['nickname']), 'rating': r['rating'],
            'content': decrypt_field(r['content']), 'createdAt': r['created_at']
        } for r in rows]
    ...
    def handle_reviews_create(self):
        ...
        if not content or len(content) > 500:
            ...
            return
        _phone_dec = decrypt_field(row['phone'])
        nickname = decrypt_field(row['nickname']) or ('游客' + _phone_dec[-4:])
        ...
        with DB_LOCK:
            ...
            try:
                cur = conn.execute(
                    'INSERT INTO spot_reviews (spot_id, user_id, nickname, rating, content, created_at) '
                    'VALUES (?,?,?,?,?,?)',
                    (spot_id, row['id'], encrypt_field(nickname), rating, encrypt_field(content), created_at))
                conn.commit()
                rid = cur.lastrowid
            finally:
                conn.close()
    ...
    def handle_chat_log(self):
        ...
        仅登录用户的对话才会存储（未登录游客不存储聊天记录），且 user_question/ai_answer 加密存储。"""
        try:
            length = int(self.headers.get('Content-Length', 0))
            body = json.loads(self.rfile.read(length) or '{}')
            ...
            if not question or not answer:
                ...
                return
            # 仅登录用户存储聊天记录；未登录用户不存储
            user_row = _get_user_by_token(self._bearer_token())
            if not user_row:
                # 未登录用户：不存储，但返回 ok 以免前端报错
                self.send_json(200, {'ok': True, 'stored': False})
                return
            with DB_LOCK:
                conn = _db()
                try:
                    conn.execute(
                        'INSERT INTO chat_logs (user_id, user_question, ai_answer, emotion, category, created_at) VALUES (?, ?, ?, ?, ?, ?)',
                        (user_row['id'], encrypt_field(question), encrypt_field(answer), emotion, category, datetime.datetime.now().isoformat())
                    )
                    conn.commit()
                finally:
                    conn.close()
            self.send_json(200, {'ok': True, 'stored': True})
        except Exception as e:
            self.send_json(500, {'error': str(e)})
    ...
    def handle_sentiment_report(self):
        ...
        try:
            ...
            self.send_json(200, {
                ...
                'negativeLogs': [{'question': decrypt_field(r[0]), 'answer': decrypt_field(r[1]), 'time': r[2]} for r in negative_logs],
            })
        except Exception as e:
            self.send_json(500, {'error': str(e)})
    ...
    def handle_admin_ai_login(self):
        """管理员登录，签发 token（从 admin_users 表验证，PBKDF2 校验）"""
        body = self._read_json_body()
        username = str(body.get('username', '')).strip()
        password = str(body.get('password', ''))
        if not _verify_admin_login(username, password):
    ...
    def handle_admin_ai_models_list(self):
        ...
        models = [{
            ...
            'role': r['role'], 'api_url': r['api_url'], 'api_key': decrypt_field(r['api_key']),
        ...
        with DB_LOCK:
            ...
            try:
                cur = conn.execute(
                    ...
                    'VALUES (?,?,?,?,?,?,?,?,?)',
                    (name, provider_type, role, api_url, encrypt_field(api_key), model_name, context_limit, status, created_at)
                )
                conn.commit()
    ...
    def handle_admin_ai_models_update(self):
            ...
            try:
                ...
                for key in ('name', 'provider_type', 'role', 'api_url', 'api_key', 'model_name'):
                    ...
                    if val is not None and str(val).strip() != '':
                        if key == 'api_key':
                            fields[key] = encrypt_field(str(val).strip())
                        else:
        ...
        self.send_json(200, {'model': {
            ...
            'role': row['role'], 'api_url': row['api_url'], 'api_key': decrypt_field(row['api_key']),
    ...
    def handle_admin_avatar_reset(self):
        ...
        with DB_LOCK:
            ...
            try:
                ...
                now = datetime.datetime.now().isoformat()
                builtin_avatars = [
                    {'name': '小乐', 'vrm_path': '/static/avatars/use_for_app1.vrm',
        ...
        try:
            ...
            for (q,) in questions:
                ...
                for cat, kws in category_keywords.items():
                    if any(kw in _q_dec for kw in kws):
            ...
            self.send_json(200, {
                ...
                'negativeLogs': [
                    {'question': decrypt_field(r[0]), 'answer': decrypt_field(r[1]), 'time': r[2]} for r in negative_logs
                ],
    ...
    def handle_ai_proxy_recognize(self):
        ...
        def _call_model(m):
            ...
            req = urllib.request.Request(
                ...
                headers={
                    'Content-Type': 'application/json',
                    'Authorization': f'Bearer {decrypt_field(m["api_key"])}'
                }
            )
    ...
    def handle_ai_proxy_tts(self):
        ...
        try:
            ...
            req = urllib.request.Request(
                ...
                headers={
                    'Content-Type': 'application/json',
                    'Authorization': f'Bearer {decrypt_field(model["api_key"])}'
                }
            )
