  ...
  <view class="auth-page">
    ...
    <view class="auth-card">
      ...
      <view class="form-item">
        ...
        <input class="input" v-model="form.password" type="password" :placeholder="currentLang === 'zh' ? '至少 6 位' : 'At least 6 characters'" />
      </view>

      <view class="form-item">
        <text class="label">{{ currentLang === 'zh' ? '确认密码' : 'Confirm Password' }}</text>
        <input class="input" v-model="form.confirm" type="password" :placeholder="currentLang === 'zh' ? '再次输入密码' : 'Confirm password'" />
      </view>

      <view class="form-item">
        <text class="label">{{ currentLang === 'zh' ? '年龄' : 'Age' }}</text>
        <input class="input" v-model="form.age" type="number" :placeholder="currentLang === 'zh' ? '用于安全路线推荐' : 'For safe route recommendation'" />
      </view>

      <view class="form-item">
        <text class="label">{{ currentLang === 'zh' ? '昵称（选填）' : 'Nickname (optional)' }}</text>
        <input class="input" v-model="form.nickname" :placeholder="currentLang === 'zh' ? '不填默认“游客+手机后4位”' : 'Visitor + last 4 digits of phone'" />
      </view>
