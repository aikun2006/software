  ...
  <view class="chat-container">
    <view class="left-panel" v-show="!isMobile || mobileAvatarVisible">
      <view class="panel-header">
        <text class="panel-title">{{ currentLang === 'zh' ? 'AI导游 - 小乐' : 'AI Guide - Xiao Le' }}</text>
      ...
      <view class="avatar-section">
        <view class="avatar-stage">
          <Avatar3D ref="avatarRef" :vertical-offset-ratio="1/4" :full-body-fit="true" :disable-zoom="true" :model-path="activeAvatarConfig.vrm_path" :model-scale="activeAvatarConfig.model_scale" :model-rotation-y="activeAvatarConfig.rotation_y" class="chat-avatar-3d" />
        </view>
    ...
    <view class="center-panel">
      <view class="panel-header chat-header">
        ...
        <view class="header-right">
          ...
          <view class="map-btn" @click="goToMap">
            <text class="map-btn-text">{{ currentLang === 'zh' ? '地图' : 'Map' }}</text>
          </view>
      ...
      >
        ...
        <view v-if="isPreparing" class="loading-indicator">
          ...
          <text class="loading-text">{{ currentLang === 'zh' ? '小乐正在思考...' : 'Xiaole is thinking...' }}</text>
        </view>
      </scroll-view>
      ...
      <view v-show="!isVoiceMode" class="chat-input-area">
        <!-- 猜你想问胶囊标签（首页专用，60秒轮换） -->
        <view class="suggest-wrap" :class="{ 'suggest-fading': suggestFading }">
          <view
      ...
      <view v-if="showImageMenu" class="overlay" @click="closeImageMenu">
        <view class="action-sheet" @click.stop="">
          <view class="action-item" @click="pickCamera"><text>📸 {{ currentLang === 'zh' ? '拍照' : 'Camera' }}</text></view>
          <view class="action-item" @click="pickGallery"><text>🖼️ {{ currentLang === 'zh' ? '从相册选择' : 'Choose from Album' }}</text></view>
          <view class="action-cancel" @click="closeImageMenu"><text>{{ currentLang === 'zh' ? '取消' : 'Cancel' }}</text></view>
        </view>
      </view>
    </view>
  </view>
...
// D5：当前启用的数字人形象配置（从后端获取，控制 VRM 模型路径/缩放/旋转/音色）
const activeAvatarConfig = ref<{
  vrm_path: string
  model_scale: number
  rotation_y: number
  voice_type: string
}>({ vrm_path: '', model_scale: 3.25, rotation_y: 0, voice_type: 'zh-CN-XiaoxiaoNeural' })
...
.suggest-wrap {
  ...
  overflow-x: hidden;         /* 超出容器隐藏 */
  gap: 0 10rpx;            /* 仅横向间距10rpx */
  padding: 12rpx 16rpx;    /* 左右留白与输入框对齐 */
  margin-bottom: 12rpx;
