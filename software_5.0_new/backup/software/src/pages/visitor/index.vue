<template>
  <view class="home-screen" @touchmove.stop.prevent>
    ...
    <view class="avatar-area" @click="onAvatarClick">
      ...
      <Avatar3D ref="avatarRef" :show-name="false" :disable-zoom="true" class="enlarged-avatar" />
    ...
    <view class="font-toggle-btn" @click="toggleLargeFont">
      <text class="font-toggle-text">{{ isLargeFont ? '标准版' : '大字版' }}</text>
    </view>
...
// 检测英文模式（通过body class判断）
const checkEnMode = () => {
  if (typeof document !== 'undefined' && document.body) {
    isEnMode.value = document.body.classList.contains('en-mode')
  }
}
// ============ 用户须知弹窗结束 ============
</script>
