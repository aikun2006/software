<template>
  <view class="detail-page">
    ...
    <view class="detail-header">
      ...
      <text class="header-title">{{ shop?.name || '美食详情' }}</text>
    ...
    <scroll-view v-if="shop" class="detail-scroll" scroll-y>
      ...
      <view class="info-card">
        <view class="info-main">
          <view class="title-row">
            <text class="spot-name">{{ shop.name }}</text>
            <text class="spot-tag" :class="shop.tagClass">{{ shop.tag }}</text>
          </view>
          <view class="meta-row">
            <text class="meta-item" v-if="shop.distance">📍 {{ shop.distance }}</text>
            <text class="meta-item" v-if="shop.price">💰 {{ shop.price }}</text>
            <text class="meta-item" v-if="shop.rating">⭐ {{ shop.rating }}分</text>
          </view>
          <view class="loc-row" v-if="shop.locationInfo">
            <text class="loc-text">{{ shop.locationInfo }}</text>
          </view>
          <view class="loc-row" v-if="shop.time">
            <text class="loc-text">🕐 {{ shop.time }}</text>
          </view>
          <!-- 特色标签 -->
          <view class="feature-tags" v-if="shop.features.length">
            <text class="feature-tag" v-for="ft in shop.features" :key="ft">{{ ft }}</text>
          </view>
        </view>
        <!-- 圆形「去这里」按钮 -->
      ...
      <view class="section-card" v-if="shop.fullDesc">
        ...
        <text class="section-text">{{ shop.fullDesc }}</text>
      </view>

      <!-- 就餐贴士 -->
      <view class="section-card tips-card" v-if="shop.tips">
        <text class="section-title">💡 就餐贴士</text>
        <text class="section-text">{{ shop.tips }}</text>
      </view>
        ...
        <view class="qa-list" :class="{ 'qa-fading': qaFading }">
          <view class="qa-item" v-for="item in qaDisplayed" :key="item.id" @click="qaToggleExpand(item.id)">
            <view class="qa-question">
              ...
              <text class="qa-q-text">{{ item.question }}</text>
            ...
            <view class="qa-answer" v-if="qaExpandedId === item.id">
              ...
              <text class="qa-a-text">{{ item.answer }}</text>
            </view>
          </view>
        </view>
      </view>
      ...
      <view class="post-card">
        ...
        <template v-else>
          <text class="login-tip">登录后即可发表评价，分享你的就餐感受</text>
...
import { getFoodById, type FoodShop, type FoodReview } from '@/data/foods'
import { reviewApi, type Review } from '@/api/review'
import { useUserStore } from '@/stores/user'
import { foodQA, useQaRotation } from '@/data/qa-bank'

const userStore = useUserStore()

const shop = ref<FoodShop | undefined>(undefined)
...
// 猜你想问
const { displayed: qaDisplayed, expandedId: qaExpandedId, fading: qaFading, toggleExpand: qaToggleExpand } = useQaRotation(foodQA)
...
// 从后端加载用户发表的评价，合并到预生成评价后
const loadApiReviews = async () => {
  try {
    const res = await reviewApi.list(shop.value!.id)
    // 后端返回的评价追加到预生成评价后面
    const apiReviews: FoodReview[] = res.reviews.map((r: Review) => ({
      id: r.id,
      nickname: r.nickname,
      rating: r.rating,
      content: r.content,
...
const goLogin = () => {
  uni.navigateTo({ url: '/pages/visitor/login?from=food:' + shop.value!.id })
}
