...
class AIResponder {
  ...
  private initDefaultRules() {
    this.replyRules = [
      ...
      {
        id: 'route_recommend',
        keywords: ['路线', '推荐', '怎么走', '游览', '行程', '攻略', '先去', '路线规划'],
        responses: [
      ...
      {
        id: 'food_recommend',
        keywords: ['吃', '吃饭', '餐厅', '美食', '推荐吃', '附近', '哪里吃', '特色菜'],
        responses: [
  ...
  getResponse(question: string, emotion?: 'positive' | 'neutral' | 'negative'): {
    ...
    for (const rule of this.replyRules) {
      ...
      if (matchCount > 0 && (matchCount > maxMatchCount || (matchCount === maxMatchCount && rule.priority > (matchedRule?.priority || 0)))) {
