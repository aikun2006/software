...
const props = withDefaults(defineProps<{
  ...
  modelUrl: '/static/avatars/puppy护士 1 侧边灰 arkit校正.vrm',
  showControls: false,
...
const initScene = async () => {
  ...
  try {
    const result = await deepProfiler.profileStage('init-scene', '场景初始化', async () => {
...
const loadVRM = async (url: string) => {
  try {
    ...
    await deepProfiler.profileStage('download-model', '模型下载', async () => {
    ...
    const parseResult = await deepProfiler.profileStage('parse-model', '模型解析', async () => {
    ...
    await deepProfiler.profileStage('init-model', '模型初始化', async () => {
      scene.add(vrm.scene)
...
onMounted(async () => {
  ...
  await deepProfiler.profileStage('total-init', '总初始化', async () => {
    await deepProfiler.profileStage('preload-shaders', 'Shader预加载', async () => {
      shaderPrecompiler.schedulePrecompile()
    })
    
    await deepProfiler.profileStage('wait-three', '等待Three.js', async () => {
      const waitThree = () => {
